...
<body style="background: #f0f9fd;">
	<div class="formbody">
    		<div class="formtitle"><span>一级菜单基本信息</span></div>
    		<form id="ff_menu_create" method="post">
				<ul class="forminfo">
					<li><label><fmt:message key="menu.id" />:</label><input name="id" type="text"
						style="width: 345px; height: 32px;" " class="easyui-textbox"
						data-options="required:true" /><i>标题不能超过30个字符</i>
					</li>
					<li><label><fmt:message key="menu.name" />:</label><input name="name" type="text"
						style="width: 345px; height: 32px;" " class="easyui-textbox"
						data-options="required:true" /><i>名称不能超过50个字符</i>
					</li>
					<li><label><fmt:message key="menu.url" />:</label><input name="url" type="text"
						style="width: 345px; height: 32px;" " class="easyui-textbox"
						data-options="required:false" /><i>示例如下：sf/menu/index</i>
					</li>
					...
					<li><label><fmt:message key="menu.description" />：</label>
					<input name="description" type="text"
						style="width: 345px; height: 80px;" " class="easyui-textbox"
						data-options="required:false,multiline:true" /><i>菜单描述不能超过200个字符</i>
					</li>
				</ul>
			</form>
	...
	function submitForm() {
		$('#ff_menu_create').ajaxSubmit({
			type : 'post',
			url : "${ctx}/${frameworkPath}/menu/create?parent=${param.parent}",
