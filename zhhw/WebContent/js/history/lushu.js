	...
	baidu.object
			.extend(
					LuShu.prototype,
					{
						...
						_addInfoWin : function() {
							...
							var overlay = new CustomOverlay(me._marker
									.getPosition(), me._opts.defaultContent);
						...
						_move : function(initPos, targetPos, effect) {
							...
							//获取结束点的(x,y)坐标
							targetPos = this._projection
									.lngLatToPoint(targetPos),
							//总的步长
							count = Math.round(me._getDistance(initPos,
									targetPos)
									/ step);
							...
							me._intervalFlag = setInterval(function() {
								//两点之间当前帧数大于总帧数的时候，则说明已经完成移动
									if (currentCount >= count) {
										...
										//正在移动
										currentCount++;
										var x = effect(initPos.x, targetPos.x,
						...
						_moveNext : function(index) {
							...
							if (index < this._path.length - 1) {
								me._move(me._path[index], me._path[index + 1],
										me._tween.linear);
							}
						},
						/**
						...
						if (index != -1) {
							...
							me._overlay.setPosition(pos,
									me._marker.getIcon().size);
					...
					//清除暂停后再开始运行的timeout
						...
						linear : function(initPos, targetPos, currentCount,
								count) {
					...
					_troughPointIndex : function(markerPoi) {
						...
						if (!t[i].bShow) {
							distance = this._map.getDistance(new BMap.Point(
									t[i].lng, t[i].lat), markerPoi);
							//两点距离小于10米，认为是同一个点
							if (distance < 10) {
								t[i].bShow = true;
								return i;
	...
	baidu.object.extend(CustomOverlay.prototype, {
		...
		setPosition : function(poi, markerSize) {
			...
			var px = this._map.pointToOverlayPixel(poi), styleW = baidu.dom
					.getStyle(this._div, 'width'), styleH = baidu.dom.getStyle(
					this._div, 'height');
