...
function initGrid(pageNum, pageSize) {
	...
	$.ajax( {
		type : "post",
		url : getContextPath() + "/history/getLine",
		data : {
			pageNum : pageNum,
			pageSize : pageSize
		},
...
function onClickRow(index) {
	...
	// 向后台发请求，更新当前地图的围栏显示
	currentLineid = $('#dg').datagrid('getSelected').id;
	var linepoints;
	//alert(currentLineid);
	$.ajax({
		type : "post",
		url : getContextPath()+"/history/getLinePoint",
		dataType : "json",
		data : {
		...
		success : function(data) {
			...
			if(data.length>0){
				var lineOverlay = new BMap.Polyline(pts);
				lineOverlay.setStrokeColor("red")//设置多边形边线颜色
				lineOverlay.setFillColor("red");//设置多边形填充颜色
				// 增加右键菜单
				var markerMenu=new BMap.ContextMenu();
				markerMenu.addItem(new BMap.MenuItem('删除',removeMarker.bind(lineOverlay)));
				markerMenu.addItem(new BMap.MenuItem('围栏信息',function(pt){$.messager.alert('围栏信息',$('#dg').datagrid('getSelected').info)}));
				lineOverlay.addContextMenu(markerMenu);
		...
		error : function(data) {
			alert(JSON.stringify(linepoints));
		}
	});
}
/*
...
function append() {
	...
	win.window({
		href: getContextPath()+'/history/toAddLine'
	});
	win.window('open');
}
/*
	...
	$.messager.confirm("确认", "确认删除", function(r) {
		if (r) {
			...
			if (row) {
				...
				$.ajax( {
					type : "post",
					url : getContextPath() + "/history/delLine",
					data : {
						"id" : row.id
					},
					dataType : "json",
...
function storePoints(ply,info){
	currentLine = $('#dg').datagrid('getSelected');
	if(currentLine == null){
...
function storePoints1(ply,replace){
	var currentLineid = $('#dg').datagrid('getSelected').id;
	var points = ply.getPath();
	// 将顶点组装成json数据
	var linepoints = "[";
	for(var i = 0;i < points.length ; i++){
		...
		linepoints = linepoints + '{"lng":"'+lng+'","lat":"'+lat+'"}';
	...
	$.ajax({
		type : "post",
		url : getContextPath()+"/history/drawLine",
		data : {
			"lineid": currentLineid,
			"linepoints" : linepoints
		},
